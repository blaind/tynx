...
use tensor::{
    ...
    full_py, index_select_py, maximum_py, minimum_py, nonzero_py, ones_like_py, ones_py,
...
fn _tynx(module: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    module.add_function(wrap_pyfunction!(argsort_py, module)?)?;
    module.add_function(wrap_pyfunction!(topk_py, module)?)?;
