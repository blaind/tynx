...
pub fn execute(node: &Node, env: &Env, device: &Device) -> Result<Vec<Value>> {
    match node {
        ...
        Node::ConvTranspose1d(node) => convolution::conv_transpose1d(node, env, device),
        Node::ConvTranspose2d(node) => convolution::conv_transpose2d(node, env, device),
        Node::ConvTranspose3d(node) => convolution::conv_transpose3d(node, env, device),
        ...
        Node::RandomNormal(node) => random::random_normal(node, device),
        Node::RandomNormalLike(node) => random::random_normal_like(node, env, device),
        Node::RandomUniform(node) => random::random_uniform(node, device),
